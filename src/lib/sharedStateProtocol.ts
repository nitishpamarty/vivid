...
export type SharedMutation =
  ...
  | { kind: 'undo'; actor: 'person'; restoreState: DashboardState; undoOfVersion: number };
...
export interface SharedMutationSuccess {
  ok: true;
  data: { state: DashboardState; version: number; activity: SharedActivity };
}
...
export type SharedMutationResult = SharedMutationSuccess | SharedMutationFailure;

export function parseSharedMutationResult(value: unknown): SharedMutationResult {
  ...
  if (result.ok === true && data?.state && typeof data.version === 'number' && activity && typeof activity.id === 'number' &&
    ...
    return { ok: true, data: { state: data.state as DashboardState, version: data.version, activity: activity as unknown as SharedActivity } };
  }
